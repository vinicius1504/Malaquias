...
import { ArrowLeft, Calendar, Tag, MessageCircle, ArrowRight, Loader2 } from 'lucide-react';
...
export default function NoticiaPage() {
  ...
  const fetchArticle = async () => {
    try {
      ...
      if (response.ok && data.news) {
        ...
        fetchRelatedArticles(data.news.category);
      } else {
        setArticle(null);
      }
    } catch (error) {
  ...
  const fetchRelatedArticles = async (category: string) => {
    try {
      const response = await fetch(`/api/news?locale=${locale}&category=${category}&limit=4`);
      const data = await response.json();

      if (response.ok && data.news) {
        // Filtrar para não incluir o artigo atual
        const filtered = data.news.filter((item: NewsItem) => item.slug !== slug).slice(0, 3);
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f5]">
      ...
      <section className="relative min-h-[50vh] flex items-end">
        {/* Background Image - usa image_banner se disponível, senão image_url */}
        <div className="absolute inset-0 overflow-hidden">
          <Image
            src={article.image_banner || article.image_url || DEFAULT_IMAGE}
            alt={article.title}
            fill
